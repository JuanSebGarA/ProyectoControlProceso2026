...
tspan = linspace(0,1500,150); % Tiempo infinito
...
%% Polos
Den_tf = Sys_tf.Denominator{1};
Polos = roots(Den_tf);
Polos2 = pole(Sys_ss);
Zeros = tzero(Sys_tf);

G_reducido = minreal(Sys_tf);

%% Controlabilidad
Controlabilidad = ctrb(A1,B1);

Cont = length(A1)-rank(Controlabilidad);
...
%% Figuras
figure(1)
set(gcf,'Units','centimeters','Position',[2 2 16 25])
